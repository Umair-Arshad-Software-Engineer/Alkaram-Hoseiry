...
class _ViewVendorsPageState extends State<ViewVendorsPage> {
  ...
  void _addBill(String vendorId, String vendorName) async {
    final languageProvider = Provider.of<LanguageProvider>(context, listen: false);
    final DatabaseReference vendorRef = FirebaseDatabase.instance.ref('vendors/$vendorId');
            ...
            return AlertDialog(
              title: Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Text(
                    languageProvider.isEnglish ? 'Add Bill' : 'بل شامل کریں',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(
                        labelText: languageProvider.isEnglish ? 'Bill Amount' : 'بل کی رقم',
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: languageProvider.isEnglish ? 'Bill Reference Number' : 'بل ریفرنس نمبر',
                        prefixIcon: Icon(Icons.receipt, color: Colors.purple),
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(8),
                        ),
                        hintText: languageProvider.isEnglish ? 'e.g., INV-2024-001' : 'مثال: INV-2024-001',
                      ),
                      onChanged: (value) {
                        billReference = value;
                      },
                    ...
                    ListTile(
                      title: Text(
                        languageProvider.isEnglish
                            ? 'Date: ${DateFormat('yyyy-MM-dd').format(billDate)}'
                            : 'تاریخ: ${DateFormat('yyyy-MM-dd').format(billDate)}',
                        style: TextStyle(fontWeight: FontWeight.bold),
                      ),
                    ...
                    ElevatedButton.icon(
                      ...
                      label: Text(languageProvider.isEnglish ? 'Upload Bill Image' : 'بل کی تصویر اپ لوڈ کریں'),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (billReference == null || billReference!.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(languageProvider.isEnglish
                              ? 'Please enter a bill reference number'
                              : 'براہ کرم بل ریفرنس نمبر درج کریں'),
                          backgroundColor: Colors.red,
                        ),
                      );
                      return;
                    ...
                    try {
                      ...
                      final billData = {
                        ...
                        'type': 'bill', // This is a credit entry
                      ...
                      // Save bill to vendor's bills node (separate from payments)
                      await vendorRef.child('bills').push().set(billData);

                      // Update vendor's total bill amount (credit)
                      await vendorRef.child('totalBills')
                          .set(ServerValue.increment(billAmount));

                      // Also save to a central bills node for easy tracking
                      await FirebaseDatabase.instance.ref('vendorBills').push().set({
                        ...billData,
                  ...
                  child: Text(languageProvider.isEnglish ? 'Save Bill' : 'بل محفوظ کریں'),
                ),
              ],
            );
          },
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      // In the trailing section of ListTile in build method, update the Row:
                      trailing: SizedBox(
                        width: 200, // Increased width to accommodate bill button
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                          children: [
                            // New Bill Button (Credit)
                            IconButton(
                              icon: Icon(Icons.receipt, color: Colors.purple),
                              onPressed: () => _addBill(vendor["id"], vendor["name"]),
                              tooltip: languageProvider.isEnglish ? 'Add Bill' : 'بل شامل کریں',
                            ),
                            // Payment Button (Debit)
                            IconButton(
