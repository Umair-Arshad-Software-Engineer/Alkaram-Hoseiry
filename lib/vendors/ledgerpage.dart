...
class _VendorLedgerPageState extends State<VendorLedgerPage> {
  ...
  Future<void> _fetchLedgerData() async {
    try {
      ...
      final DatabaseReference billsRef = FirebaseDatabase.instance.ref('vendors/${widget.vendorId}/bills'); // NEW
      ...
      // Fetch bills data (CREDIT entries)
      final billsSnapshot = await billsRef.get();
      final List<Map<String, dynamic>> bills = [];

      if (billsSnapshot.exists) {
        final billsMap = billsSnapshot.value as Map<dynamic, dynamic>;

        billsMap.forEach((billKey, billValue) {
          if (billValue is Map) {
            bills.add({
              'date': billValue['date'] ?? 'Unknown Date',
              'description': 'Bill',
              'description2': billValue['description'] ?? '',
              'credit': (billValue['amount'] ?? 0.0).toDouble(),
              'debit': 0.0,
              'type': 'credit',
              'billId': billKey,
              'billReference': billValue['billReference'] ?? '',
              'method': 'Bill',
              'image': billValue['image'] ?? '',
            });
          }
        });
      }

      // Fetch payments data (DEBIT entries)
        ...
        paymentsMap.forEach((paymentKey, paymentValue) {
          if (paymentValue is Map) {
            ...
            payments.add({
              ...
              'image': paymentValue['image'] ?? '',
            });
          }
        });
      }

      // Combine all entries: purchases, bills (both credit), and payments (debit)
      final combinedEntries = [...purchases, ...bills, ...payments];
  ...
  Widget _buildEnhancedTransactionRow(
    ...
    // Check if this is a bill entry
    final isBill = entry['description'] == 'Bill';
        ...
        child: Container(
          decoration: BoxDecoration(
            color: isOpeningBalance
                ...
                : (isBill
                ? Colors.purple.shade50  // Light purple for bills
                : backgroundColor)),
            border: Border(
          ...
          child: Row(
            children: [
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              ...
                              color: isOpeningBalance
                                  ? Colors.orange[800]
                                  : (isBill ? Colors.purple[800] : Colors.black87),
                            ),
                            overflow: TextOverflow.ellipsis,
                          ),
                          // Show reference number for bills
                          if (isBill && entry['billReference'] != null && entry['billReference'].toString().isNotEmpty)
                            Text(
                              'Ref: ${entry['billReference']}',
                              style: TextStyle(
                                fontSize: 11,
                          ...
                          // Show reference number for purchases
              ...
              _buildEnhancedDataCell(
                isBill ? 'Bill' : (entry['method'] ?? '-'),
                1.2,
                false,
                textColor: isBill ? Colors.purple[700] : Colors.grey[700],
                fontWeight: isBill ? FontWeight.bold : FontWeight.normal,
              ),
              _buildEnhancedDataCell(
                Row(
                  mainAxisSize: MainAxisSize.min,
              ...
              _buildEnhancedDataCell(
                ...
                textColor: entry['credit'] > 0
                    ? (isBill ? Colors.purple[700] : Colors.green[700])
                    : Colors.grey[500],
              ),
              _buildEnhancedDataCell(
